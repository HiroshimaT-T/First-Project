...
d.addEventListener("DOMContentLoaded", (e) => {
  hamburgerMenu(
    ".panel-btn",
    ".header-nav__container-list-desactibe",
    ".header-nav-link__items"
  );
});
...
function lluviaFontAwesome() {
  let cantidadFA = 30;
  let i = 0;
  let sectionExperence = document.querySelector(
    ".experiencia-container-moster"
  );
  while (i < cantidadFA) {
...
let linkExperiencia = document.querySelectorAll(
  ".experiencia__experiences-box"
);
...
check.addEventListener("click", () => {
  const interuptores = document.querySelectorAll(".intrp__btn");
  interuptores.forEach((interuptS) => {
    interuptS.classList.toggle("intrp__btn-on");
  });
  

  const foco = document.querySelector(".focus");
  foco.classList.toggle("focus-on");

  check.classList.toggle("checkbox-active");
  const brillitos = document.querySelectorAll(".fa-alien-monster");
  console.log(brillitos);
  brillitos.forEach((brillito) => {
    brillito.classList.toggle("fa-alien-monster-filter");
  });
});
let navigation = document.querySelector(".box-navigation")
navigation.onclick = function(){
    navigation.classList.toggle('box-navigation-active');
};
